...
[ -z "$NAMESPACE_PG_SCRET_DESTINATION" ] && error_exit "NAMESPACE_PG_SCRET_DESTINATION n'est pas défini dans .env"

# Afficher les informations de configuration
echo "🔄 Configuration:"
echo "  - Secret source: $NAME_PG_SCRET_SOURCE"
echo "  - Namespace source: $NAMESPACE_PG_SCRET_SOURCE"
echo "  - Namespace destination: $NAMESPACE_PG_SCRET_DESTINATION"
...
# Créer le namespace $NAMESPACE_PG_SCRET_DESTINATION s'il n'existe pas
kubectl create namespace $NAMESPACE_PG_SCRET_DESTINATION --dry-run=client -o yaml | kubectl apply -f - || error_exit "Impossible de créer/vérifier le namespace $NAMESPACE_PG_SCRET_DESTINATION"
...
# Créer ou mettre à jour le secret dans le namespace $NAMESPACE_PG_SCRET_DESTINATION
cat << EOF | kubectl apply -f - || error_exit "Impossible de créer/mettre à jour le secret dans le namespace $NAMESPACE_PG_SCRET_DESTINATION"
apiVersion: v1
kind: Secret
metadata:
  name: $NAME_PG_SCRET_SOURCE
  namespace: $NAMESPACE_PG_SCRET_DESTINATION
type: Opaque
data:
...
if kubectl get secret "$NAME_PG_SCRET_SOURCE" -n $NAMESPACE_PG_SCRET_DESTINATION >/dev/null 2>&1; then
    success_message "Secret créé/mis à jour avec succès dans le namespace $NAMESPACE_PG_SCRET_DESTINATION"
    echo "📝 Détails:"
    ...
    echo -n "POSTGRES_DB: "
    kubectl get secret $NAME_PG_SCRET_SOURCE -n $NAMESPACE_PG_SCRET_DESTINATION -o jsonpath='{.data.POSTGRES_DB}' | base64 --decode
    echo
    echo -n "POSTGRES_USER: "
    kubectl get secret $NAME_PG_SCRET_SOURCE -n $NAMESPACE_PG_SCRET_DESTINATION -o jsonpath='{.data.POSTGRES_USER}' | base64 --decode
    echo
    echo "POSTGRES_PASSWORD: ***" # Pour la sécurité, on n'affiche pas le mot de passe
else
    error_exit "Le secret n'a pas été créé correctement dans le namespace $NAMESPACE_PG_SCRET_DESTINATION"
fi